...
class SQLiteKnowledgeRepository(KnowledgeRepository):
    ...
    def reindex_document(self, document_id: str) -> int:
        document = self.get_document(document_id)
        if document is None:
            raise KnowledgeNotFoundError(f"Documento de conhecimento não encontrado: {document_id}")
        with self._connect() as connection:
    ...
    def _insert_document(self, connection: sqlite3.Connection, document: KnowledgeDocument) -> None:
        connection.execute(
            """
    ...
    @staticmethod
    def _insert_chunks(connection: sqlite3.Connection, chunks: tuple[KnowledgeChunk, ...]) -> None:
    ...
    def _document_from_row(row: sqlite3.Row) -> KnowledgeDocument:
        return KnowledgeDocument(
            ...
            metadata={
                str(key): str(value) for key, value in json.loads(str(row["metadata_json"])).items()
            },
        )
