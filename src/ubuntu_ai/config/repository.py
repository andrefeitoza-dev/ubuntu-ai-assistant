...
class ConfigRepository:
    ...
    def save(self, settings: AppSettings) -> None:
        ...
        settings_with_paths = (
            ...
            else settings.with_paths(self._paths)
        )
        ...
        temporary_file = self._config_file.with_suffix(
            f"{self._config_file.suffix}.tmp"
        )
    ...
    def _serialize(settings: AppSettings) -> str:
        ...
        if paths is None:
            raise ValueError(
                "Os caminhos da aplicação precisam estar definidos."
            )
        ...
        lines = [
            ...
            (
                "directory = "
                f"{ConfigRepository._quote(str(settings.logging.directory))}"
            ),
            ...
            (
                "clear_between_tasks = "
                f"{ConfigRepository._boolean(settings.ui.clear_between_tasks)}"
            ),
            "",
            "[paths]",
            (
                "config_directory = "
                f"{ConfigRepository._quote(str(paths.config_directory))}"
            ),
            (
                "data_directory = "
                f"{ConfigRepository._quote(str(paths.data_directory))}"
            ),
            (
                "cache_directory = "
                f"{ConfigRepository._quote(str(paths.cache_directory))}"
            ),
            (
                "state_directory = "
                f"{ConfigRepository._quote(str(paths.state_directory))}"
            ),
            (
                "config_file = "
                f"{ConfigRepository._quote(str(paths.config_file))}"
            ),
            "",
        ]
