...
class LoggingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if normalized_level not in valid_levels:
            raise ValueError(
                "O nível de log deve ser DEBUG, INFO, WARNING, "
                "ERROR ou CRITICAL."
            )

        if self.max_file_size_mb <= 0:
            raise ValueError(
                "O tamanho máximo do arquivo de log deve ser maior que zero."
            )

        if self.backup_count < 0:
            raise ValueError(
                "A quantidade de arquivos de backup não pode ser negativa."
            )
