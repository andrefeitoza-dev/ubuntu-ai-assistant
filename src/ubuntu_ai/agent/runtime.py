...
class AgentRuntime:
    ...
    def run(self, task: AgentTask) -> AgentResult:
        ...
        self._session_manager.remember(self._format_context_message(context))
        ...
        if self._reflection_engine is not None:
            self._plan_reflection = self._reflect_before_execution(pipeline_result.plan)
    ...
    def confirm(self) -> tuple[ExecutionResult, ...]:
        ...
        if not self._pipeline_result.plan.steps:
            raise RuntimeError("O plano não possui etapas executáveis.")
        ...
        for step_index, step in enumerate(self._pipeline_result.plan.steps):
            ...
            if self._reflection_engine is not None:
                ...
                self._session_manager.remember(f"Reflexão pós-execução: {reflection.summary()}")
    ...
    def _format_context_message(context: AgentContext) -> str:
        ...
        project_description = (
            context.project_name if context.project_name is not None else "nenhum projeto detectado"
        )
