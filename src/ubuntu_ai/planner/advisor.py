        ...
        if environment is not None:
            ...
            if environment.git_repository:
                ...
                recommendations.append(
                    "Preserve o histórico Git e evite operações destrutivas."
                )
            ...
            if environment.virtual_environment:
                recommendations.append(
                    "Utilize o ambiente virtual existente."
                )

            if environment.docker_available:
                profiles.append("Docker Ready")
                recommendations.append(
                    "Prefira containers Docker quando apropriado."
                )
            ...
            if environment.ollama_available:
                ...
                recommendations.append(
                    "Considere utilizar modelos locais via Ollama."
                )
                preferred_tools.append("ollama")

        if context.last_errors:
            risk_hints.append(
                "Existem falhas recentes registradas no contexto."
            )
            recommendations.append(
                "Evite repetir comandos que falharam recentemente."
            )

        if context.last_commands:
            recommendations.append(
                "Considere os comandos executados anteriormente."
            )
