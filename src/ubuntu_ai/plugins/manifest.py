...
class PluginManifest:
    ...
    def from_mapping(cls, data: dict[str, Any]) -> PluginManifest:
        ...
        if ":" not in entrypoint:
            raise PluginManifestError(
                "entrypoint deve usar o formato 'modulo:objeto'."
            )

        raw_permissions = data.get("permissions", ())
        if isinstance(raw_permissions, str) or not isinstance(
            raw_permissions, (list, tuple)
        ):
    ...
    def load(cls, path: str | Path) -> PluginManifest:
        ...
        try:
            if manifest_path.suffix.lower() == ".json":
                ...
                raise PluginManifestError(
                    "Formato não suportado. Use plugin.toml ou plugin.json."
                )
        except (OSError, json.JSONDecodeError, tomllib.TOMLDecodeError) as exc:
            raise PluginManifestError(
                f"Não foi possível ler o manifesto {manifest_path}."
            ) from exc
