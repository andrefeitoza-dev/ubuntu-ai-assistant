...
class PluginLoader:
    ...
    def load(self, manifest_path: str | Path) -> LoadedPlugin:
        ...
        try:
            ...
            raise PluginLoadError(f"Falha ao carregar o entrypoint de {manifest.name}.") from exc
        finally:
            if inserted:
                sys.path.remove(plugin_root)

        if not isinstance(instance, UbuntuAIPlugin):
            raise PluginLoadError(f"Entrypoint de {manifest.name} não implementa UbuntuAIPlugin.")
        try:
            instance.initialize(PluginContext())
        except Exception as exc:
            raise PluginLoadError(f"Falha ao inicializar o plugin {manifest.name}.") from exc
