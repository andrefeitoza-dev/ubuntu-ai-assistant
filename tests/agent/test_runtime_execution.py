...
def test_runtime_executes_all_steps_after_confirmation() -> None:
    ...
    assert all(result.status is ExecutionStatus.EXECUTED for result in results)
...
def test_runtime_sends_plan_commands_to_controlled_executor() -> None:
    ...
    assert controlled_executor.execute.call_args_list[:2] == [
        ...
        call(ExecutionRequest(command="sudo apt install -y docker.io")),
    ]
...
def test_runtime_stops_after_blocked_command() -> None:
    ...
    controlled_executor.execute.assert_any_call(ExecutionRequest(command="sudo apt update"))
    controlled_executor.execute.assert_any_call(
        ExecutionRequest(command="sudo apt install -y docker.io")
    )
...
def test_runtime_records_execution_results_in_session() -> None:
    ...
    assert any("Execução executada: sudo apt update" in message for message in history)
    assert any(
        "Execução bloqueada: sudo apt install -y docker.io" in message for message in history
    )
...
def test_runtime_records_failed_execution_in_session() -> None:
    ...
    assert any("Execução falhou: sudo apt update" in message for message in history)
